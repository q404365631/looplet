...
OpenHarness historically had only a single ``max_briefing_tokens``
...
* :class:`ThresholdCompactHook` — ready-to-register
  :class:`openharness.loop.LoopHook` whose ``should_compact()`` returns
...
class ThresholdCompactHook:
    ...
    :class:`openharness.compact.CompactService` on
    :class:`openharness.loop.LoopConfig` to get threshold-based
    auto-compaction:
